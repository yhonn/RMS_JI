...
Public Class frm_proyectos
    ...
    Dim frmCODE As String = "AP_ADMIN_PROY"
    ...
    Sub fillGrid(ByVal bndBind As Boolean)
        Using dbentities As New dbRMS_JIEntities
            ...
            Dim actividades = dbentities.vw_tme_ficha_proyecto.Where(Function(p) p.es_componente = False And (p.nombre_proyecto.Contains(Me.txt_doc.Text) Or
                                                                                                 ...
                                                                                                 (idProyectos.Contains(p.id_ficha_proyecto))).ToList()
            Me.grd_cate.DataSource = actividades
    ...
    Protected Sub grd_cate_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles grd_cate.ItemDataBound
        ...
        If e.Item.ItemType = GridItemType.AlternatingItem Or e.Item.ItemType = GridItemType.Item Then
            ...
            hlnkEdit.NavigateUrl = "frm_proyectosEdit?Id=" & DataBinder.Eval(e.Item.DataItem, "id_ficha_proyecto").ToString()

            Dim hlnkMod As HyperLink = New HyperLink
            hlnkMod = CType(e.Item.FindControl("col_hlk_mod"), HyperLink)
            hlnkMod.NavigateUrl = "frm_modificaciones?Id=" & DataBinder.Eval(e.Item.DataItem, "id_ficha_proyecto").ToString()
            'hlnkEdit.ToolTip = controles.iconosGrid("col_hlk_edit")


            Dim col_hlk_hito As HyperLink = New HyperLink
            col_hlk_hito = CType(e.Item.FindControl("col_hlk_hito"), HyperLink)
            col_hlk_hito.NavigateUrl = "frm_proyectoHitos?Id=" & DataBinder.Eval(e.Item.DataItem, "id_ficha_proyecto").ToString()
            'hlnkEdit.ToolTip = controles.iconosGrid("col_hlk_edit")
            ...
            Dim c_entregables = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_Entregables"))
            Dim c_Aportes = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_Aportes"))
            Dim c_Objectives = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_Objectives"))
            Dim c_Key_issues = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_Key_issues"))
            Dim c_villages = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_villages"))
            Dim c_support_services = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_support_services"))
            Dim c_indicadores = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_indicadores"))
            Dim c_documentos = Convert.ToDecimal(DataBinder.Eval(e.Item.DataItem, "c_documentos"))
            'If c_entregables > 0 And c_Aportes > 0 And c_Objectives > 0 And c_Key_issues > 0 And _
            '    c_villages > 0 And c_support_services > 0 And c_indicadores > 0 And c_documentos > 0 Then
            '    hlnkCompleto.ImageUrl = "../Imagenes/iconos/accept.png"
            '    'hlnkCompleto.ToolTip = controles.iconosGrid("col_hlk_completo")
            '    hlnkCompleto.ToolTip = "Complete"
            'End If

            If c_Aportes > 0 Then
                hlnkMod.Visible = True
            End If

            If c_entregables > 0 And c_Aportes > 0 And c_Objectives > 0 And c_indicadores > 0 And c_documentos > 0 Then
                hlnkCompleto.ImageUrl = "../Imagenes/iconos/accept.png"
                'hlnkCompleto.ToolTip = controles.iconosGrid("col_hlk_completo")
                hlnkCompleto.ToolTip = "Complete"
            End If
            ...
            If DataBinder.Eval(e.Item.DataItem, "georeferencia_completa").ToString().Equals("NO") Then
                ...
                'hlnk_OrdenInicioCMP.Enabled = False
            End If
    ...
    Protected Sub btn_nuevo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_nuevo.Click
        Me.Response.Redirect("~/Proyectos/frm_proyectosAD")
    End Sub
    ...
    Public Function get_Proyectos() As Integer()
        Using dbEntities As New dbRMS_JIEntities
            ...
            For Each item In oProyecto
                ...
                Dim A As String() = item.id_subregion.Split(",")
