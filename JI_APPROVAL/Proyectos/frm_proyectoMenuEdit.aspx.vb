...
Public Class frm_proyectoMenuEdit
    ...
    Sub CargarDatos(ByVal IdProyecto As String)
        ...
        Dim dmMenu As New SqlDataAdapter("SELECT * FROM tme_FichaProyectoOrdenInicio WHERE id_ficha_proyecto=" & IdProyecto & " ORDER BY id_ficha_proyecto_ordInicio DESC", cnnME)
        Dim dsMenu As New DataSet("dsMenu")
        dmMenu.Fill(dsMenu, "dsMenu")
        'Me.lbl_fechaOI.Text = dsMenu.Tables("dsMenu")(0)("fecha_ordInicio")
        Me.lbl_id_rep.Text = dsMenu.Tables("dsMenu")(0)("id_ficha_proyecto_ordInicio")

        Me.grd_Instrumentos2.DataBind()

        Dim dmIns As New SqlDataAdapter("SELECT id_indicador FROM tme_FichaProyectoOrdenInicioIndicador WHERE id_ficha_proyecto_ordInicio=" & Me.lbl_id_rep.Text, cnnME)
        Dim dsIns As New DataSet("dsMenu")
        dmIns.Fill(dsIns, "dsMenu")
        Me.grd_Instrumentos2.DataBind()
        For Each row In Me.grd_Instrumentos2.Items
            For j As Integer = 0 To dsIns.Tables("dsMenu").Rows.Count() - 1
                If TypeOf row Is GridDataItem Then
                    ...
                    If id_indicador = dsIns.Tables("dsMenu")(j)("id_indicador") Then
                        Dim ChkHerramientas As CheckBox = CType(row.Cells(0).FindControl("ChkHerramientas"), CheckBox)
                        ChkHerramientas.Checked = True
                    End If
                End If
            Next
        Next

        Dim dmIndicador As New SqlDataAdapter("SELECT id_indicador FROM tme_meta_indicador_ficha WHERE requiere_aprobacion='SI' AND id_ficha_proyecto=" & IdProyecto, cnnME)
        Dim dsIndicador As New DataSet("dsMenuIndicador")
        dmIndicador.Fill(dsIndicador, "dsMenuIndicador")

        For Each row In Me.grd_Instrumentos2.Items
            For j As Integer = 0 To dsIndicador.Tables("dsMenuIndicador").Rows.Count() - 1
                If TypeOf row Is GridDataItem Then
                    Dim dataItem As GridDataItem = CType(row, GridDataItem)
                    Dim id_indicador As Integer = Convert.ToInt32(dataItem.GetDataKeyValue("id_indicador"))
                    If id_indicador = dsIndicador.Tables("dsMenuIndicador")(j)("id_indicador") Then
        ...
        Dim dmInsMenu As New SqlDataAdapter("SELECT id_instrumento FROM tme_FichaProyectoMenu WHERE id_ficha_proyecto=" & IdProyecto, cnnME)
        Dim dsInsMenu As New DataSet("dsInsMenu")
        dmInsMenu.Fill(dsInsMenu, "dsInsMenu")
        Me.grd_Instrumentos3.DataBind()

        For Each row In Me.grd_Instrumentos3.Items
            For j As Integer = 0 To dsInsMenu.Tables("dsInsMenu").Rows.Count() - 1
                If TypeOf row Is GridDataItem Then
                    Dim dataItem As GridDataItem = CType(row, GridDataItem)
                    Dim id_instrumento As Integer = Convert.ToInt32(dataItem.GetDataKeyValue("id_instrumento"))
                    If id_instrumento = dsInsMenu.Tables("dsInsMenu")(j)("id_instrumento") Then
                        Dim ChkHRSeguimiento As CheckBox = CType(row.Cells(0).FindControl("ChkHRSeguimiento"), CheckBox)
                        ChkHRSeguimiento.Checked = True
                    End If
                End If
            Next
        Next
    ...
    Protected Sub btn_guardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        Try
            Using dbEntities As New dbRMS_HNEntities
                ...
                dbEntities.Database.ExecuteSqlCommand("DELETE FROM tme_FichaProyectoMenu WHERE id_ficha_proyecto=" & idProyecto)
                dbEntities.Database.ExecuteSqlCommand("DELETE FROM tme_FichaProyectoOrdenInicioIndicador WHERE id_ficha_proyecto_ordInicio= " & Me.lbl_id_rep.Text)
                ...
                For Each row In Me.grd_Instrumentos2.Items
                    If TypeOf row Is GridDataItem Then
                        ...
                        If ChkHerramientas.Checked = True Then
                            dbEntities.Database.ExecuteSqlCommand("INSERT INTO tme_FichaProyectoOrdenInicioIndicador(id_ficha_proyecto_ordInicio, id_indicador) VALUES(" & lbl_id_rep.Text & "," & id_indicador & ")")
                        End If
