...
Public Class frm_menulenguageAD
    Inherits System.Web.UI.Page

    Dim cl_user As ly_SIME.CORE.cls_user
    Dim frmCODE As String = "AP_LANG_MENU"
    ...
    Protected Sub btn_guardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btn_guardar.Click
        ...
        If errSave = False Then
            ...
            Using dbEntities As New dbRMS_JIEntities
                ...
                Dim controles = dbEntities.t_menu_idioma.ToList()
                For Each row In Me.grd_cate.Items
                    If TypeOf row Is GridDataItem Then
                        Dim dataItem As GridDataItem = CType(row, GridDataItem)
                        id_menu = Convert.ToInt32(dataItem.GetDataKeyValue("id_menu"))
                        ItemD = CType(row, GridDataItem)
                        Dim observaciones As RadTextBox = CType(ItemD.FindControl("txt_valor"), RadTextBox)

                        If controles.Where(Function(p) p.id_menu = id_menu).Count() > 0 Then
                            Dim oMeta = dbEntities.t_menu_idioma.Where(Function(p) p.id_menu = id_menu And p.id_idioma = Me.cmb_idioma.SelectedValue)
                            If oMeta.Count() > 0 Then
                                Dim menuIdioma = oMeta.FirstOrDefault()
                                menuIdioma.valor = observaciones.Text
                                dbEntities.Entry(menuIdioma).State = Entity.EntityState.Modified
                            Else
                                Dim oMenuIdioma = New t_menu_idioma
                                oMenuIdioma.id_menu = id_menu
                                oMenuIdioma.id_idioma = Me.cmb_idioma.SelectedValue
                                oMenuIdioma.valor = observaciones.Text

                                dbEntities.t_menu_idioma.Add(oMenuIdioma)
                            End If
                        Else
                            Dim oMenuIdioma = New t_menu_idioma
                            oMenuIdioma.id_menu = id_menu
    ...
    Sub FillData()
        ...
        Try REM por si no hay una planificacion
            Using dbEntities As New dbRMS_JIEntities
                Dim idiomaMenu = dbEntities.t_menu_idioma.ToList()
                Me.grd_cate.DataSource = dbEntities.vw_t_menu.ToList()
                ...
                For Each row In Me.grd_cate.Items
                    If TypeOf row Is GridDataItem Then
                        Dim dataItem As GridDataItem = CType(row, GridDataItem)
                        Dim id_menu As Integer = Convert.ToInt32(dataItem.GetDataKeyValue("id_menu"))
                        ItemD = CType(row, GridDataItem)
                        Dim observaciones As RadTextBox = CType(ItemD.FindControl("txt_valor"), RadTextBox)
                        If idiomaMenu.Where(Function(p) p.id_menu = id_menu And p.id_idioma = Me.cmb_idioma.SelectedValue).Count() > 0 Then
                            observaciones.Text = idiomaMenu.FirstOrDefault(Function(p) p.id_menu = id_menu And p.id_idioma = Me.cmb_idioma.SelectedValue).valor
                        End If
                    End If
                Next
            End Using
