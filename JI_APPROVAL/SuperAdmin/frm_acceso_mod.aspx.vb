...
Imports ly_SIME
Public Class frm_acceso_mod
    Inherits System.Web.UI.Page

    Dim cl_user As ly_SIME.CORE.cls_user
    Dim frmCODE As String = "AP_ADM_ROLMOD"
    ...
    Sub fillGrid()
        ...
        Dim id_rol = Convert.ToInt32(Me.Request.QueryString("Id").ToString)
        Me.grd_cate.DataSource = db.VW_GR_USER_MOD.Where(Function(p) p.id_rol = id_rol And (p.mod_name.Contains(Me.txt_doc.Text) Or p.mod_url.Contains(Me.txt_doc.Text) Or p.mod_desc.Contains(Me.txt_doc.Text))).OrderBy(Function(p) p.sys_p).ThenBy(Function(p) p.sys_name).ToList()
        Dim rol = db.t_rol.Find(id_rol)
        Me.lbl_subtitulo_aux.Text = "-" & rol.rol_name
    ...
    Protected Sub grd_cate_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles grd_cate.ItemDataBound
        If e.Item.ItemType = GridItemType.AlternatingItem Or e.Item.ItemType = GridItemType.Item Then
            ...
            hlnkAcceso_ctrl.NavigateUrl = "frm_acceso_ctrl?Id=" & DataBinder.Eval(e.Item.DataItem, "id_mod").ToString() & "&IdRol=" & Me.lbl_id_rol.Text
            ...
            visible.Attributes.Add("idAcceso", DataBinder.Eval(e.Item.DataItem, "id_acc_mod").ToString())
    ...
    Protected Sub btn_nuevo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_nuevo.Click
        If Me.cmb_id_mod.SelectedValue IsNot Nothing Then
            Using dbEntities As New dbRMS_JIEntities
                Dim oAccesMod As New t_access_mod

                oAccesMod.id_rol = Convert.ToInt32(Me.lbl_id_rol.Text)
                oAccesMod.id_mod = Convert.ToInt32(Me.cmb_id_mod.SelectedValue)

                'Para denotar que pertenece al sistema SIME
                oAccesMod.acm_acc = 1

                dbEntities.t_access_mod.Add(oAccesMod)
                dbEntities.SaveChanges()
            End Using
            'Me.MsgGuarda.Visible = True
            Me.Response.Redirect(String.Concat("~/SuperAdmin/frm_acceso_mod?id=", Me.lbl_id_rol.Text))
        End If
    End Sub
