...
Partial Class frm_TimeSheetDocs
    Inherits System.Web.UI.Page

    Dim cl_user As ly_SIME.CORE.cls_user
    Dim frmCODE As String = "AP_TIMESHEET_DOCS"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Me.hd_IDtimeSheet.Value = Request.QueryString("ID")
    ...
    Public Sub loadData()
        ...
        Dim tbl_Suppor_docs As DataTable = cls_TimeSheet.TimeSheet_Support_Docs(hd_IDtimeSheet.Value)
        ...
        Me.anchorSupportDocs.Attributes.Add("class", "btn btn-primary btn-circle")
        ...
        Me.cmb_approvals.DataSource = tbl_Approval
        ...
        If tbl_Approval.Rows.Count > 0 Then
            ...
            If tbl_result.Rows.Count > 0 Then
                ...
                Me.grd_documentos.DataSource = cls_TimeSheet.get_Doc_support_Route_TS(Me.cmb_approvals.SelectedValue, If(Me.hd_files_selected.Value = "0", "", Me.hd_files_selected.Value))
                Me.grd_documentos.DataBind()

            Else


                '********************************FOR SELECTING BY DEFAULT THE TS*************************************
                Dim bndFound As Boolean = False
                If timesheet.ts_leave_update = True Then

                    For Each row As DataRow In tbl_Approval.Rows

                        If Convert.ToInt32(row("is_leave_approval")) = 1 Then
                            Me.cmb_approvals.SelectedValue = row("id_tipoDocumento")
                            bndFound = True
                        End If
                ...
                Else
                    ...
                    For Each row As DataRow In tbl_Approval.Rows
                        ...
                        If Convert.ToInt32(row("is_leave_approval")) = 0 Then
                            Me.cmb_approvals.SelectedValue = row("id_tipoDocumento")
                            bndFound = True
                        End If
        ...
        If tot_files > 0 Then
            ...
            Me.grd_documentos.DataSource = cls_TimeSheet.get_Doc_support_Route_TimeSheet(tbl_result.Rows.Item(0).Item("id_tipoDocumento"), If(Me.hd_files_selected.Value = "0", "", Me.hd_files_selected.Value))
            Me.grd_documentos.DataBind()
    ...
    Private Sub btnlk_continue_Click(sender As Object, e As EventArgs) Handles btnlk_continue.Click
        ...
        Try
            ...
            Using db As New dbRMS_JIEntities
                ...
                cls_TimeSheet = New APPROVAL.clss_TimeSheet(Me.Session("E_IDPrograma"))
                Dim idTimeSheet As Integer = Convert.ToUInt32(Me.hd_IDtimeSheet.Value)

                Dim ResultTimeSheets As Integer = db.ta_documento_timesheets.Where(Function(p) p.id_timesheet = idTimeSheet).Count()
                Dim DocTs As ta_documento_timesheets

                If ResultTimeSheets = 0 Then
                    ...
                    'DocTs.id_documento = 0

                    If Val(Me.hd_id_documento.Value) > 0 Then
                        DocTs.id_documento = Me.hd_id_documento.Value
                    End If

                    cls_TimeSheet.SaveDocumento_TimeSheet(DocTs, 0)

                Else

                    DocTs = db.ta_documento_timesheets.Find(Convert.ToInt32(Me.hd_id_documento_timesheet.Value))
                ...
                If flg_Err Then
                    Me.Response.Redirect("~/TimeSheet/frm_TimeSheetFollowing.aspx?ID=" & Me.hd_IDtimeSheet.Value)
                Else 'Error
                    Me.lblError.Text = String.Format("Error saving Docs, please contact to the system administrator <br /><br />")
        ...
        Catch ex As Exception
            Me.lblError.Text = String.Format("Error saving TimeSheet Docs, please contact to the system administrator <br /><br /> Detail:{0}", ex.Message)
            Me.lblError.Visible = True
        End Try
    ...
    Protected Sub chkVisible_CheckedChangedDOCS(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If rdListBox_files.Items.Count <> Convert.ToInt32(Me.hd_has_files.Value) Then 'means that something has changed
            ...
            If Val(Me.cmb_approvals.SelectedValue) > 0 Then
                ...
                'cls_Deliverable = New APPROVAL.clss_Deliverable(Me.Session("E_IDPrograma"))
                cls_TimeSheet = New APPROVAL.clss_TimeSheet(Me.Session("E_IDPrograma"))
                Me.grd_documentos.DataSource = cls_TimeSheet.get_Doc_support_Route_TS(Val(Me.cmb_approvals.SelectedValue), If(Me.hd_files_selected.Value = "0", "", Me.hd_files_selected.Value))
                Me.grd_documentos.DataBind()
    ...
    Private Sub cmb_approvals_SelectedIndexChanged(sender As Object, e As RadComboBoxSelectedIndexChangedEventArgs) Handles cmb_approvals.SelectedIndexChanged
        ...
        If e.Value IsNot Nothing Then
            Me.grd_documentos.DataSource = cls_TimeSheet.get_Doc_support_Route_TS(e.Value, If(Me.hd_files_selected.Value = "0", "", Me.hd_files_selected.Value))
            Me.grd_documentos.DataBind()
        End If
    ...
    Public Sub Check_del_Ones()
        ...
        If Convert.ToInt32(Me.hd_has_files.Value) > 0 Then 'Do we need to looking for the deleted Ones
            ...
            Dim i As Integer = 0
            tbl_Suppor_docs = cls_TimeSheet.TimeSheet_Support_Docs(Me.hd_IDtimeSheet.Value)
            ...
            For Each dtRow In tbl_Suppor_docs.Rows
                ...
                If boolDeleted Then 'It was already deleted by DB
                    ...
                    If DelFileParam(dtRow("archivo").ToString.Trim, "\FileUploads\ApprovalProcc\") Then
                        'Delete from the DB
                        cls_TimeSheet.TimeSheet_Support_Docs_del(dtRow("id_timesheet_support_docs"))
            ...
            If Convert.ToBoolean(Me.lbl_hasFiles.Value) Then 'if it has values
                ...
                For Each item As RadListBoxItem In rdListBox_files.Items
                    ...
                    If Not boolFind Then
                        ...
                        File = String.Format("doc{0}_0{1}_{2}_{3}{4}{5}", Me.hd_IDtimeSheet.Value, Me.Session("E_IdUser"), dmyhm, FileOrginal.Replace("'", "").Replace("`", "").Replace("´", "").Replace("á", "a").Replace("é", "e").Replace("í", "i").Replace("ó", "o").Replace("ú", "u").Replace(" ", "_").Replace("%", "-").Replace(".", "_").Replace(",", "-").Replace("&", "-").Replace("'", "").Replace("""", "").Replace("#", ""), "_v1.1", extension)
                        ...
                        Try
                            ...
                            Dim tbl_ta_timesheet_support_docs As New ta_timesheet_support_docs

                            tbl_ta_timesheet_support_docs.id_timesheet = Me.hd_IDtimeSheet.Value
                            tbl_ta_timesheet_support_docs.archivo = File
                            tbl_ta_timesheet_support_docs.id_doc_soporte = CType(item.Value, Integer)
                            tbl_ta_timesheet_support_docs.ver = 1

                            If cls_TimeSheet.Save_TimeSheet_support_docs(tbl_ta_timesheet_support_docs, 0) <> -1 Then
                                'Error here
                            End If
