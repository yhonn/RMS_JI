...
Public Class frm_viaje_legalizacionSgmt
    Inherits System.Web.UI.Page
    Dim cl_user As ly_SIME.CORE.cls_user
    Dim cl_error As New ly_SIME.CORE.ErrorHandler
    Dim frmCODE As String = "ADM_VIAJES_LGL_SGMT"
    ...
    Sub LoadData(ByVal id_viaje As Integer)
        ...
        Using dbEntities As New dbRMS_JIEntities
            ...
            If legalizacion IsNot Nothing Then
                Dim intOwnerLegalizacion As String() = legalizacion.id_usuario_app_legalizacion.ToString.Split(",")
                If intOwnerLegalizacion.Where(Function(p) p.Contains(indx)).Count() = 0 Then
                    Me.Response.Redirect("~/administrativo/frm_viaje_legalizacionPrint?id=" & id_viaje)
                End If
            End If
            ...
            If viaje.fecha_finalizacion_viaje_legalizacion IsNot Nothing Then
                Me.lbl_fecha_hora_finalizacion.Text = viaje.fecha_finalizacion_viaje_legalizacion.Value.Year & "-" & viaje.fecha_finalizacion_viaje_legalizacion.Value.Month & "-" & viaje.fecha_finalizacion_viaje_legalizacion.Value.Day & " " & viaje.hora_fin_viaje
            End If

            'Dim fechaInicio As DateTime = New DateTime(viaje.fecha_inicio_viaje_legalizacion.Value.Year, viaje.fecha_inicio_viaje_legalizacion.Value.Month,
            '                                                    viaje.fecha_inicio_viaje_legalizacion.Value.Day, Me.rt_hora_inicio.SelectedTime.Value.Hours,
            '                                                    Me.rt_hora_inicio.SelectedTime.Value.Minutes, Me.rt_hora_inicio.SelectedTime.Value.Seconds)

            'Dim fechaFin As DateTime = New DateTime(viaje.fecha_finalizacion_viaje_legalizacion.Value.Year, viaje.fecha_finalizacion_viaje_legalizacion.Value.Month,
            '                                                    viaje.fecha_finalizacion_viaje_legalizacion.Value.Day, Me.rt_hora_fin.SelectedTime.Value.Hours,
            ...
            '    Me.lbl_duracion.Text = "Viaje de corta duración (" & diferencia.Hours & " horas)"
            'Else
            '    Me.lbl_duracion.Text = "Viaje mayor a 12 horas"
            'End If
                            ...
                            If clss_approval.save_ta_AppDocumento() <> -1 Then
                                ...
                                If idRuta = -1 Then ' there is not more steps
                                    ...
                                    If clss_approval.save_ta_documento() <> -1 Then
                                        ...
                                        If (objEmail.Emailing_APPROVAL_TRAVEL_LEGALIZATION(CType(id_app_documento, Integer), id_viaje)) Then
                                        Else 'Error mandando Email
                                        End If
                                ...
                                Else 'Yes there is more steps
                                    ...
                                    If Tool_code = "TM-SHEET01" And clss_approval.get_TimeSheetDoc(Me.HiddenField1.Value) > 0 Then '--Time Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************NEXT STEP APP****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), Me.HiddenField1.Value, 1021, cl_user.regionalizacionCulture, id_appdocumento)
                                    ...
                                    ElseIf Tool_code = "DELIV-RMS01" Then '--Deliverable Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************APPROVED****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1021, cl_user.regionalizacionCulture, id_appdocumento)
                                    ...
                                    ElseIf Tool_code = "TRAVEL-RM01" Then '--Deliverable Tools
                                        ...
                                        '**************  Estatus 2 *********** Approval in Process ***********************************************************
                                        Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1021, cl_user.regionalizacionCulture, id_appdocumento)
                                        If (objEmail.Emailing_APPROVAL_TRAVEL_LEGALIZATION(id_appdocumento, id_viaje)) Then
                                        Else 'Error mandando Email
                                        End If
                                    Else 'No tool related to this approval

                                        '*********************************APPROVED NEXT STEP****************************************
                                        Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), Me.HiddenField1.Value, 1021, cl_user.regionalizacionCulture, CType(id_appdocumento, Integer))
                                        If (objEmail.Emailing_APPROVAL_STEP(CType(id_appdocumento, Integer))) Then
                                        Else 'Error mandando Email
                                        End If
                                    End If
                                    '*********************************APPROVED NEXT STEP****************************************
