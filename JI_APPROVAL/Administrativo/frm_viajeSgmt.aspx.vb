...
Public Class frm_viajeSgmt
    Inherits System.Web.UI.Page
    Dim cl_user As ly_SIME.CORE.cls_user
    Dim cl_error As New ly_SIME.CORE.ErrorHandler
    Dim frmCODE As String = "ADM_VIAJES_SGMT"
    ...
    Sub LoadData(ByVal id_viaje As Integer)
        ...
        Using dbEntities As New dbRMS_JIEntities
            ...
            Dim viajeDetalle = dbEntities.vw_tme_solicitud_viaje.Where(Function(p) p.id_viaje = id_viaje).FirstOrDefault()
            Dim id_usuario_app = viajeDetalle.id_usuario_app
            Dim id_usuario_solicita = viajeDetalle.id_usuario
            Dim indx As Integer = Convert.ToInt32(Me.Session("E_IDUser"))

            If viajeDetalle IsNot Nothing Then
                Dim intOwnerLegalizacion As String() = viajeDetalle.id_usuario_app.ToString.Split(",")
                If intOwnerLegalizacion.Where(Function(p) p.Contains(indx)).Count() = 0 Then
                    Me.Response.Redirect("~/administrativo/frm_viajeDetalle2?id=" & id_viaje)
                End If
            End If

            Me.lbl_categoria.Text = viaje.ta_documento_viaje.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().ta_documento.ta_tipoDocumento.descripcion_aprobacion
            ...
            'If id_usuario_app = indx And id_usuario_solicita = indx And viajeDetalle.id_estadoDoc = 6 Then
            If intOwner.Where(Function(p) p.Contains(indx)).Count() > 0 And id_usuario_solicita = indx And viajeDetalle.id_estadoDoc = 6 Then
                Me.Response.Redirect(String.Format("~/administrativo/frm_viajeEdit.aspx?Id={0}", Request.QueryString("id"))) 'Redirect
            End If
                            ...
                            If clss_approval.save_ta_AppDocumento() <> -1 Then
                                ...
                                If idRuta = -1 Then ' there is not more steps
                                    ...
                                    If clss_approval.save_ta_documento() <> -1 Then
                                        ...
                                        If (objEmail.Emailing_APPROVAL_TRAVEL(id_app_documento, id_viaje)) Then
                                        Else 'Error mandando Email
                                        End If
                                        ' ********************************COMPLETED DOCUMENT*********************************************************
