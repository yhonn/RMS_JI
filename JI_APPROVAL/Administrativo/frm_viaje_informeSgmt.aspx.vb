...
Public Class frm_viaje_informeSgmt
    Inherits System.Web.UI.Page
    Dim cl_user As ly_SIME.CORE.cls_user
    Dim cl_error As New ly_SIME.CORE.ErrorHandler
    Dim frmCODE As String = "ADM_VIAJES_INFO_SGMT"
    ...
    Sub LoadData(ByVal id_viaje As Integer)
        ...
        Using dbEntities As New dbRMS_JIEntities
            ...
            Me.lblt_conclusiones.Text = viaje.informe_compromiso
            Dim informe = dbEntities.vw_tme_solicitud_viaje.Where(Function(p) p.id_viaje = id_viaje).FirstOrDefault()

            If informe IsNot Nothing Then
                Dim intOwnerLegalizacion As String() = informe.id_usuario_app_informe.ToString.Split(",")
                If intOwnerLegalizacion.Where(Function(p) p.Contains(indx)).Count() = 0 Then
                    Me.Response.Redirect("~/administrativo/frm_viaje_informePrint?id=" & id_viaje)
                End If
            End If
            ...
            Me.lblt_lugares_entidades_personas.Text = viaje.lugares_entidades_personas
                            ...
                            If clss_approval.save_ta_AppDocumento() <> -1 Then
                                ...
                                If idRuta = -1 Then ' there is not more steps
                                    ...
                                    If clss_approval.save_ta_documento() <> -1 Then
                                        ...
                                        Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1022, cl_user.regionalizacionCulture, CType(id_app_documento, Integer))
                                ...
                                Else 'Yes there is more steps
                                    ...
                                    If Tool_code = "TM-SHEET01" And clss_approval.get_TimeSheetDoc(Me.HiddenField1.Value) > 0 Then '--Time Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************NEXT STEP APP****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), Me.HiddenField1.Value, 10, cl_user.regionalizacionCulture, CType(id_appdocumento, Integer))
                                    ...
                                    ElseIf Tool_code = "DELIV-RMS01" Then '--Deliverable Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************APPROVED****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1009, cl_user.regionalizacionCulture, CType(id_appdocumento, Integer))
                                    ...
                                    ElseIf Tool_code = "TRAVEL-RM01" Then '--Deliverable Tools
                                        ...
                                        '**************  Estatus 2 *********** Approval in Process ***********************************************************
                                        Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1022, cl_user.regionalizacionCulture, id_appdocumento)
                                        If (objEmail.Emailing_APPROVAL_TRAVEL_INFORME(id_appdocumento, id_viaje)) Then
                                        Else 'Error mandando Email
                                        End If
                                ...
                                If clss_approval.save_ta_documento() <> -1 Then
                                    ...
                                    ElseIf Tool_code = "TRAVEL-RM01" Then '--Deliverable Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************APPROVED****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1022, cl_user.regionalizacionCulture, CType(id_app_documento, Integer))
                                            If (objEmail.Emailing_APPROVAL_TRAVEL_INFORME(CType(id_app_documento, Integer), CType(id_viaje, Integer))) Then
                                            Else 'Error mandando Email
                                            End If
                                            '*********************************APPROVED****************************************
