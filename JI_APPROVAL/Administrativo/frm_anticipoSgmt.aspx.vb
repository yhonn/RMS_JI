...
Public Class frm_anticipoSgmt
    ...
    Sub LoadData(ByVal id_anticipo As Integer)
        ...
        Using dbEntities As New dbRMS_JIEntities
            ...
            Me.lbl_categoria.Text = anticipo.ta_documento_anticipos.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().ta_documento.ta_tipoDocumento.descripcion_aprobacion
            ...
            If anticipo.ta_documento_anticipos.Where(Function(p) p.reversado Is Nothing).Count() > 0 Then
                Me.lyHistory.Visible = True
                Dim idDoc = anticipo.ta_documento_anticipos.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().id_documento
                Me.HiddenField1.Value = anticipo.ta_documento_anticipos.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().id_documento

                fillGridRutaAprobacion(anticipo.ta_documento_anticipos.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().id_documento)
