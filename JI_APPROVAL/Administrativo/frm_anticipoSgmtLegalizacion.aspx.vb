...
Public Class frm_anticipoSgmtLegalizacion
    Inherits System.Web.UI.Page
    Dim cl_user As ly_SIME.CORE.cls_user
    Dim cl_error As New ly_SIME.CORE.ErrorHandler
    Dim frmCODE As String = "ADM_ANTICIPO_SGMT"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Me.IsPostBack Then
            ...
            Me.idAnticipo.Value = id_anticipo
    ...
    Sub LoadData(ByVal id_anticipo As Integer)
        If Session("dtRutas") IsNot Nothing Then
            dtRutas = Session("dtRutas")
        Else
            createdtcolumsRutas()
        ...
        Using dbEntities As New dbRMS_JIEntities
            Dim anticipo = dbEntities.tme_anticipos.Find(id_anticipo)
            Dim anticipoDetalle = dbEntities.vw_tme_anticipos.Where(Function(p) p.id_anticipo = id_anticipo).FirstOrDefault()
            Dim id_usuario_app = anticipoDetalle.id_usuario_app
            Dim id_usuario_solicita = anticipoDetalle.id_usuario
            Dim indx As Integer = Convert.ToInt32(Me.Session("E_IDUser"))

            If anticipoDetalle IsNot Nothing Then
                Dim intOwnerLegalizacion As String() = anticipoDetalle.id_usuario_app.ToString.Split(",")
                If intOwnerLegalizacion.Where(Function(p) p.Contains(indx)).Count() = 0 Then
                    Me.Response.Redirect("~/administrativo/frm_anticipoDetalle?id=" & id_anticipo)
                End If
            End If

            Me.lbl_categoria.Text = anticipo.ta_documento_anticipo_legalizacion.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().ta_documento.ta_tipoDocumento.descripcion_aprobacion
            Me.lbl_codigo.Text = anticipo.codigo_anticipo
            Me.lbl_fecha_solicitud.Text = String.Format("{0:MM/dd/yyyy}", anticipo.fecha_crea)
            Me.lbl_fecha_requiere_anticipo.Text = String.Format("{0:MM/dd/yyyy}", anticipo.fecha_anticipo)
            Me.lbl_codigo_par.Text = anticipoDetalle.codigo_par
            Me.lbl_motivo.Text = anticipoDetalle.motivo
            Me.lbl_tipo_anticipo.Text = anticipoDetalle.tipo_par

            Dim anticipoRutas = dbEntities.vw_tme_anticipo_eventos.Where(Function(p) p.id_anticipo = id_anticipo).ToList()
            ...
            End If
            For Each item In anticipo.tme_anticipo_ruta.ToList()
                Dim fecha = DateTime.Now
                Dim rnd As New Random()
                Dim aleatorio As String = ""
                Dim index = 1
                aleatorio = rnd.Next(index, 9999999).ToString()
                Dim idunique = fecha.Year & fecha.Month & fecha.Day & fecha.Hour & fecha.Minute & fecha.Second & fecha.Millisecond & aleatorio & item.id_anticipo_ruta


                dtRutas.Rows.Add(idunique, item.id_ruta, item.tme_rutas.tme_ciudades_rutas.ciudad, item.tme_rutas.tme_ciudades_rutas1.ciudad, item.tme_rutas.zona_rural, item.tme_rutas.tiempo_estimado,
                             item.cantidad_personas, item.valor_trayecto, item.total_trayecto, item.tme_rutas.observaciones, item.requiere_estipendio, item.estipendio_desayuno,
                             item.estipendio_almuerzo, item.estipendio_cena, item.cantidad_desayuno, item.cantidad_almuerzo, item.cantidad_cena,
                             item.id_lugar_estipendio, If(item.id_lugar_estipendio Is Nothing, Nothing, item.tme_lugar_estipendio.lugar),
                             item.cantidad_desayuno * item.valor_unitario_deayuno, item.cantidad_almuerzo * item.valor_unitario_almuerzo, item.cantidad_cena * item.valor_unitario_cena,
                             ((item.cantidad_desayuno * item.valor_unitario_deayuno) + (item.cantidad_almuerzo * item.valor_unitario_almuerzo) + (item.cantidad_cena * item.valor_unitario_cena)),
                             (((item.cantidad_desayuno * item.valor_unitario_deayuno) + (item.cantidad_almuerzo * item.valor_unitario_almuerzo) + (item.cantidad_cena * item.valor_unitario_cena)) * item.cantidad_personas),
                             item.total_ruta, If(item.requiere_estipendio = True, "SI", "NO"), If(item.estipendio_desayuno = True, "SI", "NO"), If(item.estipendio_almuerzo = True, "SI", "NO"), If(item.estipendio_cena = True, "SI", "NO"),
                             item.observaciones, True, item.id_anticipo_ruta, item.valor_unitario_deayuno, item.valor_unitario_almuerzo, item.valor_unitario_cena)

            Next

            For Each item In anticipo.tme_anticipo_compra.ToList()
                Dim fecha = DateTime.Now
                Dim rnd As New Random()
                Dim aleatorio As String = ""
                Dim index = 1
                aleatorio = rnd.Next(index, 9999999).ToString()
                Dim idunique = fecha.Year & fecha.Month & fecha.Day & fecha.Hour & fecha.Minute & fecha.Second & fecha.Millisecond & aleatorio & item.id_anticipo_compra


                dtCompras.Rows.Add(idunique, item.cantidad, item.descripcion, item.precio_unitario, item.valor_total, True, item.id_anticipo_compra)

            Next
            Session("dtRutas") = dtRutas
            Session("dtCompras") = dtCompras
            If anticipo.ta_documento_anticipo_legalizacion.Where(Function(p) p.reversado Is Nothing).Count() > 0 Then
                Me.lyHistory.Visible = True
                Dim idDoc = anticipo.ta_documento_anticipo_legalizacion.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().id_documento
                Me.HiddenField1.Value = anticipo.ta_documento_anticipo_legalizacion.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().id_documento

                fillGridRutaAprobacion(anticipo.ta_documento_anticipo_legalizacion.Where(Function(p) p.reversado Is Nothing).FirstOrDefault().id_documento)
            ...
            Dim Path As String
            Path = dbEntities.t_programa_settings.FirstOrDefault(Function(p) p.id_programa = id_programa).documents_folder

            If anticipo.soporte_legalizacion_eventos IsNot Nothing Then
                Me.doc_admon_content.Visible = True
                Me.doc_legalizacion.NavigateUrl = Path & anticipo.soporte_legalizacion_eventos
            End If
            Dim intOwner As String() = id_usuario_app.ToString.Split(",")


            'If id_usuario_app = indx And id_usuario_solicita = indx And anticipoDetalle.id_estadoDoc = 6 Then
            If intOwner.Where(Function(p) p.Contains(indx)).Count() > 0 And id_usuario_solicita = indx And anticipoDetalle.id_estadoDoc = 6 Then
                Me.Response.Redirect(String.Format("~/administrativo/frm_anticiposEdit.aspx?Id={0}", Request.QueryString("id"))) 'Redirect
            End If

            If anticipoDetalle IsNot Nothing Then
                Dim intOwnerLegalizacion As String() = anticipoDetalle.id_usuario_app.ToString.Split(",")
                If intOwnerLegalizacion.Where(Function(p) p.Contains(indx)).Count() = 0 Then
                    Me.infoApp.Visible = False
                End If
            End If
            'If id_usuario_app.Where(Function(p) p.co <> indx Then '--The User is not Allowed
            '    Me.infoApp.Visible = False
            'End If
        End Using
    End Sub
        ...
        Using dbEntities As New dbRMS_JIEntities
            Dim participantes = (From a In dbEntities.tme_anticipo_ruta_participantes
                                 ...
                                 Select a.numero_documento, a.primer_apellido, a.segundo_apellido, a.valor, a.nombres,
                                     a.tipo_ocumento, a.telefono, b.num_ruta, a.id_participante, a.id_ruta_anticipo, a.retiro_fondos, a.retiro_fondos_text).ToList()
    ...
    Protected Sub btn_salir_Click(sender As Object, e As EventArgs) Handles btn_salir.Click
        Me.Response.Redirect("~/administrativo/frm_anticipos")
    End Sub
                            ...
                            If clss_approval.save_ta_AppDocumento() <> -1 Then
                                ...
                                If idRuta = -1 Then ' there is not more steps
                                    ...
                                    If clss_approval.save_ta_documento() <> -1 Then
                                        ...
                                        '***************Ver la categoria y ver si aplica registro ambiental*****************************************
                                        '********************************COMPLETED DOCUMENT*********************************************************
                                        Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), Me.HiddenField1.Value, 1027, cl_user.regionalizacionCulture, CType(id_app_documento, Integer))
                                        If (objEmail.Emailing_APPROVAL_Anticipo(id_app_documento, id_anticipo)) Then
                                        Else 'Error mandando Email
                                        End If
                                        ' ********************************COMPLETED DOCUMENT*********************************************************
                                ...
                                Else 'Yes there is more steps
                                    ...
                                    If Tool_code = "TM-SHEET01" And clss_approval.get_TimeSheetDoc(Me.HiddenField1.Value) > 0 Then '--Time Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************NEXT STEP APP****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), Me.HiddenField1.Value, 10, cl_user.regionalizacionCulture, id_appdocumento)
                                            If (objEmail.Emailing_TIME_SHEET_APPROVAL(CType(Me.HiddenField1.Value, Integer))) Then
                                            Else 'Error mandando Email
                                            End If
                                            '*********************************NEXT STEP APP****************************************
                                        End If
                                    ...
                                    ElseIf Tool_code = "DELIV-RMS01" Then '--Deliverable Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************APPROVED****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1009, cl_user.regionalizacionCulture, id_appdocumento)
                                            If (objEmail.Emailing_DELIVERABLE_APPROVAL(CType(Me.HiddenField1.Value, Integer))) Then
                                            Else 'Error mandando Email
                                            End If
                                            '*********************************APPROVED****************************************
                                    ...
                                    ElseIf Tool_code = "TRAVEL-RM01" Then '--Deliverable Tools
                                        ...
                                        '**************  Estatus 2 *********** Approval in Process ***********************************************************
                                        Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1027, cl_user.regionalizacionCulture, id_appdocumento)
                                        If (objEmail.Emailing_APPROVAL_Anticipo(id_appdocumento, id_anticipo)) Then
                                        Else 'Error mandando Email
                                        End If

                                    ElseIf Tool_code = "ANT-RMS01" Then '--Deliverable Tools
                            ...
                            If clss_approval.save_ta_AppDocumento() <> -1 Then
                                ...
                                If clss_approval.save_ta_documento() <> -1 Then
                                    ...
                                    ElseIf Tool_code = "DELIV-RMS01" Then '--Deliverable Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************APPROVED****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1027, cl_user.regionalizacionCulture, CType(id_app_documento, Integer))
                                    ...
                                    ElseIf Tool_code = "TRAVEL-RM01" Then '--Deliverable Tools
                                        ...
                                        If result <> -1 Then
                                            '*********************************APPROVED****************************************
                                            Dim objEmail As New APPROVAL.cls_notification(Me.Session("E_IDPrograma"), idDoc, 1027, cl_user.regionalizacionCulture, CType(id_app_documento, Integer))
                                            If (objEmail.Emailing_APPROVAL_Anticipo(CType(id_app_documento, Integer), CType(id_anticipo, Integer))) Then
                                            Else 'Error mandando Email
                                            End If
                                            '*********************************APPROVED****************************************
