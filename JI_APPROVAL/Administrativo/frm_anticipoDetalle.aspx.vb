...
Public Class frm_anticipoDetalle
    ...
    Sub loadData(ByVal idAnticipo As Integer)
        Using dbEntities As New dbRMS_JIEntities
            ...
            Dim anticipoTb = dbEntities.tme_anticipos.Where(Function(p) p.id_anticipo = idAnticipo).FirstOrDefault()
            ...
            Me.grd_cate.DataSource = clss_approval.get_ta_RutaSeguimiento(anticipo.id_documento_legalizacion.ToString())
            Me.grd_cate.DataBind()


            Me.grd_ruta_solicitud.DataSource = clss_approval.get_ta_RutaSeguimiento(anticipo.id_documento.ToString())
            Me.grd_ruta_solicitud.DataBind()
            ...
            Dim participantes = (From a In dbEntities.tme_anticipo_ruta_participantes
                                 ...
                                 Select a.numero_documento, a.primer_apellido, a.segundo_apellido, a.valor, a.nombres,
                                     a.tipo_ocumento, a.telefono, b.num_ruta, a.id_participante, a.id_ruta_anticipo, a.valor_estipendio,
                                     a.valor_trayecto, a.retiro_fondos_text).ToList()


            Me.grd_participantes.DataSource = participantes
            Me.grd_participantes.DataBind()

            Dim Path As String
            Path = dbEntities.t_programa_settings.FirstOrDefault(Function(p) p.id_programa = id_programa).documents_folder


            Me.doc_df.NavigateUrl = Path & anticipoTb.soporte_verificacion_fondos
            Me.doc_legalizacion.NavigateUrl = Path & anticipoTb.soporte_legalizacion_eventos
            Me.lbl_observaciones.Text = anticipoTb.observaciones_dipsersion_fondos
        End Using
    End Sub
