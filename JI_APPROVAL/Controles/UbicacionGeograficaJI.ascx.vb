...
Imports ly_SIME
Imports ly_APPROVAL
Public Class UbicacionGeograficaJI
    ...
    'Public Property grupo_dv_cmb_village() As HtmlGenericControl
    '    Get
    '        Return dv_cmb_village
    '    End Get
    '    Set(ByVal value As HtmlGenericControl)
    '        dv_cmb_village = value
    '    End Set
    'End Property

    Public Property grupo_dv_cmb_country() As HtmlGenericControl
        Get
            Return dv_cmb_country
        End Get
        Set(ByVal value As HtmlGenericControl)
            dv_cmb_country = value
        End Set
    End Property


    Private listadosSubRegion As String

    Public Property listadosSubRegions() As String
        Get
            Return listadosSubRegion
        End Get
        Set(ByVal value As String)
            listadosSubRegion = value
        End Set
    End Property
    ...
    Public Property value_rv_cmb_division() As RequiredFieldValidator
        Get
            Return rv_cmb_division
        End Get
        Set(ByVal value As RequiredFieldValidator)
            rv_cmb_division = value
        End Set
    End Property

    'Public Property value_rv_cmb_district() As RequiredFieldValidator
    '    Get
    '        Return rv_cmb_district
    '    End Get
    '    Set(ByVal value As RequiredFieldValidator)
    '        rv_cmb_district = value
    '    End Set
    'End Property

    'Public Property value_rv_cmb_upazila() As RequiredFieldValidator
    '    Get
    '        Return rv_cmb_upazila
    '    End Get
    '    Set(ByVal value As RequiredFieldValidator)
    '        rv_cmb_upazila = value
    '    End Set
    'End Property

    'Public Property value_rv_cmb_union() As RequiredFieldValidator
    '    Get
    '        Return rv_cmb_union
    '    End Get
    '    Set(ByVal value As RequiredFieldValidator)
    ...
    'Public Property value_rv_cmb_village() As RequiredFieldValidator
    '    Get
    '        Return rv_cmb_village
    '    End Get
    '    Set(ByVal value As RequiredFieldValidator)
    '        rv_cmb_village = value
    '    End Set
    'End Property

    'Public Property value_rv_cmb_parish() As RequiredFieldValidator
    '    Get
    '        Return rv_cmb_parish
    '    End Get
    '    Set(ByVal value As RequiredFieldValidator)
    '        rv_cmb_parish = value
    '    End Set
    ...
    Public Property value_controlScrollGIS() As HiddenField
        Get
            Return controlScrollGIS
        End Get
        Set(ByVal value As HiddenField)
            controlScrollGIS = value
        End Set
    End Property
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Using dbEntities As New dbRMS_JIEntities
                ...
                '    villageID = idVillage
                '    oVillage = dbEntities.vw_t_village.Where(Function(p) p.id_village = villageID).ToList()
                'Else
                '    oVillage = Nothing
                'End If

                Dim countriesList = dbEntities.t_departamentos.Where(Function(p) p.id_programa = id_programa) _
                ...
                If Me.cmb_country.SelectedValue <> "" Or villageID <> 0 Then
                    ...
                    zoneList = (From div In dbEntities.tme_subregion_nivel_avance
                                Join dist In dbEntities.t_districts On div.id_relacion Equals dist.id_district
                                Join subR In dbEntities.t_subregiones On div.id_subregion Equals subR.id_subregion
                                Join Region In dbEntities.t_regiones On Region.id_region Equals subR.id_region
                                Where (dist.id_programa = id_programa And Region.id_programa = id_programa And dist.id_pais = countryID)
                    ...
                    zones = dbEntities.t_regiones.Where(Function(p) zoneList.Contains(p.id_region)).OrderBy(Function(p) p.nombre_region) _
                                             .Select(Function(p) _
                                                                New With {Key .id_region = p.id_region,
                                                                          Key .nombre_region = p.nombre_region}).ToList()
                    ...
                    If villageID <> 0 Then
                        ...
                        'zoneID = oVillage.FirstOrDefault.id_region
                        zoneID = dbEntities.t_regiones.Where(Function(p) zoneList.Contains(p.id_region)).OrderBy(Function(p) p.nombre_region) _
                                             .Select(Function(p) _
                                                                New With {Key .id_region = p.id_region,
                                                                          Key .nombre_region = p.nombre_region}).FirstOrDefault.id_region
                    Else

                        'p.id_pais = countryID And 
                        'zoneID = dbEntities.t_regiones.Where(Function(p) p.id_region = oVillage.FirstOrDefault.id_region).OrderBy(Function(p) p.nombre_region).Distinct.FirstOrDefault.id_region
                        zoneID = dbEntities.t_regiones.Where(Function(p) zoneList.Contains(p.id_region)).OrderBy(Function(p) p.nombre_region).FirstOrDefault.id_region
                ...
                Else
                    ...
                    zoneList = (From div In dbEntities.tme_subregion_nivel_avance
                                Join dist In dbEntities.t_departamentos On div.id_relacion Equals dist.id_departamento
                                Join subR In dbEntities.t_subregiones On div.id_subregion Equals subR.id_subregion
                                Join Region In dbEntities.t_regiones On Region.id_region Equals subR.id_region
                                Where (dist.id_programa = id_programa And Region.id_programa = id_programa And countriesList.Contains(dist.id_pais))
                                Select Region.id_region).Distinct().ToList()
                    ...
                    'zoneID = dbEntities.t_regiones.Where(Function(p) countriesList.Contains(p.id_pais)).OrderBy(Function(p) p.nombre_region).FirstOrDefault.id_region
                ...
                Dim divisionList As List(Of Integer)

                '*********************CHANGES COMES HERE**********************************
                'If Not String.IsNullOrEmpty(listadosSubRegions) Then
                '    Dim listado = listadosSubRegions.Split(",")
                '    distritos = distritos.Where(Function(p) listado.Contains(p.id_district)).ToList()
                'End If
                ...
                If Me.cmb_zone.SelectedValue <> "" Or villageID <> 0 Then
                    ...
                    divisionList = (From div In dbEntities.tme_subregion_nivel_avance
                                    Join dist In dbEntities.t_departamentos On div.id_relacion Equals dist.id_departamento
                                    Join subR In dbEntities.t_subregiones On div.id_subregion Equals subR.id_subregion
                                    Where (dist.id_programa = id_programa And countriesList.Contains(dist.id_pais) And subR.id_region = zoneID)
                                    Select subR.id_subregion).Distinct().ToList()

                    division = dbEntities.t_subregiones.Where(Function(p) p.id_region = zoneID And divisionList.Contains(p.id_subregion)).OrderBy(Function(p) p.nombre_subregion) _
                                         .Select(Function(p) _
                    ...
                    divisionID = dbEntities.t_subregiones.Where(Function(p) p.id_region = zoneID And divisionList.Contains(p.id_subregion)).OrderBy(Function(p) p.nombre_subregion).FirstOrDefault.id_subregion
                ...
                Else
                    ...
                    divisionList = (From div In dbEntities.tme_subregion_nivel_avance
                                    Join dist In dbEntities.t_departamentos On div.id_relacion Equals dist.id_departamento
                                    Join subR In dbEntities.t_subregiones On div.id_subregion Equals subR.id_subregion
                                    Where (dist.id_programa = id_programa And countriesList.Contains(dist.id_pais))
                                    Select subR.id_subregion).Distinct().ToList()

                    division = dbEntities.t_subregiones.Where(Function(p) divisionList.Contains(p.id_region)).OrderBy(Function(p) p.nombre_subregion) _
                                           .Select(Function(p) _
                                                   New With {Key .id_subregion = p.id_subregion,
                                                              Key .nombre_subregion = p.nombre_subregion}).ToList()


                    divisionID = dbEntities.t_subregiones.Where(Function(p) p.id_region = zoneID And divisionList.Contains(p.id_subregion)).FirstOrDefault().id_subregion
                ...
                Dim oVillages = New List(Of t_villages)
                ...
                'If Me.cmb_parish.SelectedValue <> "" Or villageID <> 0 Then
                '    Dim parishID = If(Me.cmb_parish.SelectedValue <> "", Convert.ToInt32(Me.cmb_parish.SelectedValue), oVillage.FirstOrDefault.id_parish)
                '    oVillages = dbEntities.t_villages.Where(Function(p) p.id_parish = parishID).OrderBy(Function(p) p.nombre_village).ToList()
                'Else
                '    oVillages = oParishes.FirstOrDefault().t_villages.OrderBy(Function(p) p.nombre_village).ToList()
                'End If
                ...
                'If Me.cmb_parish.SelectedValue = "" And villageID <> 0 Then
                '    Me.cmb_parish.SelectedValue = oVillage.FirstOrDefault.id_parish
                'End If

                'If Me.cmb_village.SelectedValue = "" And villageID <> 0 Then
                '    Me.cmb_village.SelectedValue = oVillage.FirstOrDefault.id_village
                'End If
    ...
    Sub LoadList(Optional i_pais = "", Optional i_zone = "", Optional i_division = "", Optional i_district = "", Optional i_county = "", Optional i_subcounty = "", Optional i_parish = "")
        ...
        If Not String.IsNullOrEmpty(i_pais) Then
            ...
            Me.cmb_zone.DataSource = clListados.get_t_regiones2(id_programa, i_pais)
            Me.cmb_zone.DataTextField = "nombre_region"
            Me.cmb_zone.DataValueField = "id_region"
            Me.cmb_zone.DataBind()
            Me.cmb_zone.Text = ""

            ' ClearValues(Me.cmb_zone)
            ClearValues(Me.cmb_division)
            ClearValues(Me.cmb_district)
            ClearValues(Me.cmb_upazila)
            'ClearValues(Me.cmb_union)
            'ClearValues(Me.cmb_parish)
        ...
        If Not String.IsNullOrEmpty(i_zone) Then
            ...
            If Not IsNothing(Me.cmb_country.SelectedValue) Then
                idPAis = Convert.ToInt32(Me.cmb_country.SelectedValue)
            End If

            Me.cmb_division.DataSource = clListados.get_t_subregiones3(i_zone, idPAis, id_programa)
            Me.cmb_division.DataTextField = "nombre_subregion"
            Me.cmb_division.DataValueField = "id_subregion"
            Me.cmb_division.DataBind()
        ...
        If Not String.IsNullOrEmpty(i_division) Then
            ...
            If idPAis = 0 Then
                Me.cmb_district.DataSource = clListados.get_t_departamentos2(i_division)
            Else
                Me.cmb_district.DataSource = clListados.get_t_departamentos3(i_division, idPAis)
            End If

            Me.cmb_district.DataTextField = "nombre_departamento"
            Me.cmb_district.DataValueField = "id_departamento"
            Me.cmb_district.DataBind()
            Me.cmb_district.Text = ""
        ...
        If Not String.IsNullOrEmpty(i_district) Then
            ...
            Me.cmb_upazila.DataSource = clListados.get_t_municipio(i_district)
        ...
        '    Me.cmb_parish.DataSource = clListados.get_t_parish(i_subcounty)
        '    Me.cmb_parish.DataTextField = "nombre_parish"
        '    Me.cmb_parish.DataValueField = "id_parish"
        '    Me.cmb_parish.DataBind()
        '    Me.cmb_parish.Text = ""
        ...
        'If Not String.IsNullOrEmpty(i_parish) Then

        '    Me.cmb_village.DataSource = clListados.get_t_villages(i_parish)
        '    Me.cmb_village.DataTextField = "nombre_village"
        '    Me.cmb_village.DataValueField = "id_village"
        '    Me.cmb_village.DataBind()
        '    Me.cmb_village.Text = ""
    ...
    'Protected Sub cmb_parish_SelectedIndexChanged(sender As Object, e As RadComboBoxSelectedIndexChangedEventArgs) Handles cmb_parish.SelectedIndexChanged
    '    LoadList(i_parish:=Me.cmb_parish.SelectedValue)
    'End Sub
